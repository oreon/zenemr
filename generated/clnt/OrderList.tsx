...
import {orderHeaders, createSchema, orderUISchema} from './Order'





export class OrderListWrapper extends React.Component<any, any> {
...
export class OrderList extends React.Component<any, any> {
  ...
  async load() {
    this.response = await DataService.load('orders');
 ...
 render() {
    ...
    return (
      ...
      <div>
        ...
        <SimpleList headers= {orderHeaders} editLink={'Order'}
...
export class OrderView extends EditViewBase {
  ...
  render() {
    ...
    return (
     <Layout>
       <SimpleView  headers= {orderHeaders} renderExtra={this.renderExtra}
       record={record}   entityName='Order' /> 
...
export class OrderEdit extends EditViewBase {
  ...
  async onSubmit(formData) {
    ...
    try {
      await DataService.onSubmit('orders', formData)
      
      //if(!this.props.prev)
     hashHistory.push('/admin/OrderList?msg=success')
      //else 
     ...
     <Layout>
      <div>
        ...
        <SimpleForm formData={this.state.record} currentError={this.state.error}
          formSchema={createSchema() }  uiSchema={orderUISchema}
